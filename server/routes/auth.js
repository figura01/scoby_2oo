...
router.post("/signin", (req, res, next) => {
    ...
    User.findOne({email}).then((userDocument) => {
        ...
        if (!userDocument) { // Status 400 for bad request.
...
router.post("/signup", (req, res, next) => {
    ...
    User.findOne({email}).then((userDocument) => {
        ...
        try { // hashSync can fail, we have to wrap the hashing in a try/catch block.
...
router.get("/isLoggedIn", (req, res, next) => { // If currentUser is defined in the session it means the user is logged in.
...
router.get("/logout", (req, res, next) => { // Well...
